...
export default function Header() {
  ...
  return (
    <header className="bg-slate-200 shadow-md w-full top-0 left-0">
      ...
      <div className="shadow-md w-full fixed top-0 left-0">
        <div className="md:flex items-center justify-between bg-slate-200 py-4 md:px-10 px-7">
          <div className="font-bold text-2xl cursor-pointer flex items-center gap-3">
            <Link
              to="/"
              className="flex items-center object-cover hover:scale-105 duration-300"
            >
              ...
              <img
                ...
                src="https://firebasestorage.googleapis.com/v0/b/real-estate-7f0dd.appspot.com/o/App_don%E2%80%99t%20delete%2FBest%20domivka_gray.png?alt=media&token=0363d869-6be7-47b2-a54d-945e68b814e5"
                alt="Logo"
              />
            </Link>

            <SignedOut className="">
              <SignInButton className=" w-6 h-6 text-xs" />
            </SignedOut>
            <SignedIn className="">
              <UserButton className=" w-6 h-6 text-xs" />
            </SignedIn>

            <form
              onSubmit={handleSubmit}
              className="bg-slate-100 rounded-lg flex items-center w-36 sm:w-44 justify-between"
            >
              <input
                type="text"
          ...
          <ul
            className={`md:flex md:items-center md:pb-0 pb-4 absolute md:static bg-slate-200 md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${
          ...
          >
            {
              <div className="flex flex-col my-4 md:flex md:flex-row md:ml-4 md:my-0 font-semibold md:gap-3">
                <Link to="/profile" className="m-3 md:m-0">
                  {currentUser ? (
                    <img
                      className="rounded-full h-7 w-7 object-cover hover:scale-125 duration-300"
                ...
                {/* <div className="flex ml-3 md:ml-4 m-3 md:m-0">
                  <button disabled={i18next.language === "uk"}>
                    <img
                      className="rounded-full h-7 w-7 object-cover hover:scale-125 duration-300"
                      onClick={() => changeLanguage("uk")}
                      src="https://upload.wikimedia.org/wikipedia/commons/d/d2/Flag_of_Ukraine.png"
                      alt="Українська"
                    />
                  </button>
                </div>
                <div className="flex ml-3 md:ml-4 m-3 md:m-0">
                  <button disabled={i18next.language === "en"}>
                    <img
                      className="rounded-full h-7 w-7 object-cover hover:scale-125 duration-300"
      ...
      <div className="flex md:flex justify-between items-center max-w-6xl mx-auto p-3 h-[7vh] py-4 md:px-10 px-7"></div>
    </header>
